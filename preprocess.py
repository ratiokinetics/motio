...
if Path("activations.npy").exists():
    acts = np.load("activations.npy", mmap_mode="r")
else:
    ...
    acts = np.lib.format.open_memmap("activations.npy", mode="w+", dtype=np.float32, shape=(len(wiki), 512, 768))
    with torch.no_grad():
        for i in range(0, len(wiki), 32):
            print("Processing: ", i, "of", len(wiki))
            batch = torch.tensor(wiki[i:i+32]["ids"], device=device)
            out = model(batch, output_hidden_states=True)
            acts[i:i+32] = out.hidden_states[LAYER].cpu().numpy()
    acts.flush()
